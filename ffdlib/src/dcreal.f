**==dcreal.spg  processed by SPAG 5.00N  at 11:12 on 23 Mar 1999
      SUBROUTINE DCREAL(Fpn)
      IMPLICIT NONE
C
C ======================================================================
C
C  PURPOSE:
C       Decode one real number.
C
C  METHOD:
...
C       - NOTE: Decoding will be in DOUBLE PRECISION, but converted to
C         REAL before exit of routine. (DFPN --> FPN)
C         If decoding of DOUBLE PRECSION: Use DCDBL
C
C  ERROR HANDLING:
...
C
C  OUTPUT:
C       FPN  - Decoded real number, if no error.
C
...
C
C ----------------------------------------------------------------------
99001 FORMAT (/,' *** FFDLIB ERROR - Call to MOVEIP - (MODULE: DCREAL)')
99002 FORMAT (/,' *** FFDLIB ERROR - Call to DCINT - (MODULE: DCREAL)')
99003 FORMAT (/,' *** FFDLIB ERROR - Invalid character ',
     &        '- (MODULE: DCREAL)',/)
99004 FORMAT (/,' *** FFDLIB WARNING - Exponent > +35, ',
     &        'number set to 1.E+35 ','- (MODULE: DCREAL)',/)
99005 FORMAT (/,' *** FFDLIB WARNING - Exponent < -35, ',
     &        'number set to 1.E-35 ','- (MODULE: DCREAL)',/)
C ----------------------------------------------------------------------
C
      END
