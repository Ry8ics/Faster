**==getcom.spg  processed by SPAG 5.00N  at 11:12 on 23 Mar 1999
      SUBROUTINE GETCOM(Ityp,Ipar)
      IMPLICIT NONE
C
C ======================================================================
C
C  PURPOSE:
C       Obtain copy of INTEGER variable in internal COMMON /FFD1/ for
C       ffdlib.a.
C
C  METHOD:
C
C  INPUT:
C      ITYP - Indicate which INTEGER variable in COMMON to obtain a
C             copy of.
C
C  OUTPUT:
...
C ----------------------------------------------------------------------
C
      IF ( Ityp.EQ.1 ) Ipar = IDCerr
      IF ( Ityp.EQ.2 ) Ipar = INP
      IF ( Ityp.EQ.3 ) Ipar = LPU
      IF ( Ityp.EQ.4 ) Ipar = IP
      IF ( Ityp.EQ.5 ) Ipar = NCOl
      IF ( Ityp.EQ.6 ) Ipar = NLIn
C
C ----------------------------------------------------------------------
      RETURN
C ----------------------------------------------------------------------
